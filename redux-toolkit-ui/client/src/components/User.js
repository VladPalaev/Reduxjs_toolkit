...
import { updateInput } from '../redux/slice/userSlice'
	...
	return (
		<div className='flex flex-col'>
			<input
				...
				onChange={ (e) => dispatch(updateInput({firstName: e.target.value})) }
				className='w-full p-1 mb-2 focus:outline-none focus:border-lime-500 focus: border-2 placeholder:text-sm'
			/>
			<input
				type='text'
				placeholder='Second Name'
				onChange={ (e) => dispatch(updateInput({lastName: e.target.value}))}
				className='w-full p-1 mb-2 focus:outline-none focus:border-lime-500 focus: border-2 placeholder:text-sm'
			/>
